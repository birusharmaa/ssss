...
if (!function_exists('validate_header_by_cat')) {
    ...
    function validate_header_by_cat($header, $category, $subcategory)
    {
        ...
        $header = "[" . $header . "]";
        if ($subcategory == "")
            $oldHeader = $ci->LeadModel->getHeaderByCat($category);
        else
            $oldHeader = $ci->LeadModel->getHeaderBySubCat($subcategory);
        if ($oldHeader != $header)
            return true;
        else
            return false;
...
function getFollowUpDays($followUpDays = null)
{
